...
describe "Redis::Store::Strategy::Marshal" do
  ...
  it "unmarshals on get" do
    @store.get("rabbit").should eql(@rabbit)
  end

  it "marshals on set" do
    @store.set "rabbit", @white_rabbit
    @store.get("rabbit").should eql(@white_rabbit)
  end

  if RUBY_VERSION.match /1\.9/
    it "doesn't unmarshal on get if raw option is true" do
      @store.get("rabbit", :raw => true).should eql("\x04\bU:\x0FOpenStruct{\x06:\tnameI\"\nbunny\x06:\x06EF")
    end
  else
    it "doesn't unmarshal on get if raw option is true" do
      @store.get("rabbit", :raw => true).should eql("\004\bU:\017OpenStruct{\006:\tname\"\nbunny")
    end
  end

  it "doesn't marshal on set if raw option is true" do
    @store.set "rabbit", @white_rabbit, :raw => true
    @store.get("rabbit", :raw => true).should eql(%(#<OpenStruct color="white">))
  end

  it "doesn't unmarshal if get returns an empty string" do
    @store.set "empty_string", ""
    @store.get("empty_string").should eql("")
    # TODO use a meaningful Exception
    # lambda { @store.get("empty_string").should eql("") }.wont_raise Exception
  end
  ...
  if RUBY_VERSION.match /1\.9/
    it "doesn't unmarshal on multi get if raw option is true" do
      ...
      rabbit.should eql("\x04\bU:\x0FOpenStruct{\x06:\tnameI\"\nbunny\x06:\x06EF")
      rabbit2.should eql("\x04\bU:\x0FOpenStruct{\x06:\ncolorI\"\nwhite\x06:\x06EF")
    end
  else
    it "doesn't unmarshal on multi get if raw option is true" do
      @store.set "rabbit2", @white_rabbit
      rabbit, rabbit2 = @store.mget "rabbit", "rabbit2", :raw => true
      rabbit.should eql("\004\bU:\017OpenStruct{\006:\tname\"\nbunny")
      rabbit2.should eql("\004\bU:\017OpenStruct{\006:\ncolor\"\nwhite")
    end
  end
